...
def show_possible_matches(my_word):
    ...
    for i in range(len(wordlist)):
        if (len(my_word) == len(wordlist[i])):
            my_word_l = list(my_word)
            wordlist_l = list(wordlist[i])
            for j in range(len(my_word_l)):
                if (my_word_l[j] == "_"):
                    wordlist_l[j] = "_"
            if (my_word_l == wordlist_l):
                print("Possible Match:",wordlist[i])
